...
class Budget(models.Model,baseModel):
	...
	def getDetails(self):
		...
		return (categories.annotate(total= Coalesce(models.Sum('budget_category__total',filter = budget_filter,distinct = True),0.0),
									total_spent = Coalesce(models.Sum('expense__value',filter = (expense_owner_filter & date_time_filter), distinct= True),0.0))).values('name','icon','total','total_spent').order_by('-total')
	...
	@classmethod
	def getBudgetsFrom(cls,from_date,user):
		date_filter = Q(budget__month__gt = from_date)
		return Budget_Category.objects.all().filter(date_filter).filter(Q(budget__user = user)).values('budget__month').annotate(total_budget = Coalesce(models.Sum('total',filter = Q(budget__user = user)),0.0)).order_by('budget__month')
	
	@classmethod
	def getBudgetsOfPeriod(cls,from_date,up_to_date,user):
